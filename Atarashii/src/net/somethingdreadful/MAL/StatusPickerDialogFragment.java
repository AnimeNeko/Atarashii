...
public class StatusPickerDialogFragment extends SherlockDialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		view = View.inflate(new ContextThemeWrapper(getActivity(),
				R.style.AlertDialog), R.layout.dialog_status_picker, null);
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int whichButton) {
						((DetailView) getActivity())
								.onStatusDialogDismissed(currentStatus);
						dismiss();
					}
				})
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (state == null) {
			...
			if ("anime".equals(type)) {
				currentStatus = ((DetailView) getActivity()).animeRecord
						.getWatchedStatus();
			} else {
				currentStatus = ((DetailView) getActivity()).mangaRecord
						.getReadStatus();
			}
		} else {
		...
		if ((Anime.STATUS_WATCHING.equals(currentStatus))
				|| (Manga.STATUS_READING.equals(currentStatus))) {
		...
		if ((Anime.STATUS_PLANTOWATCH.equals(currentStatus))
				|| (Manga.STATUS_PLANTOREAD.equals(currentStatus))) {
