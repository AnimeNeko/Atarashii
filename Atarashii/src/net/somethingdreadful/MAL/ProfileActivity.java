...
public class ProfileActivity extends SherlockFragmentActivity implements
		UserNetworkTaskFinishedListener {
	MALManager mManager;
	Context context;
	ImageView Image;
	PrefManager prefs;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new UserNetworkTask(context, forcesync, this).execute(getIntent()
		...
		tv25.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Uri webstiteclick = Uri.parse(record.getProfile().getDetails()
						.getWebsite());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.forceSync:
			if (MALApi.isNetworkAvailable(context)) {
				Crouton.makeText(this, R.string.crouton_info_SyncMessage,
						Style.INFO).show();
				forcesync = true;
				String username;
				if (record != null)
				...
				Crouton.makeText(this, R.string.crouton_error_noConnectivity,
						Style.ALERT).show();
			}
			break;
		case R.id.action_ViewMALPage:
			Uri malurl = Uri.parse("http://myanimelist.net/profile/"
					+ record.getName());
	...
	public void card() { // settings for hide a card and text userprofile
		...
		if (prefs.anime_manga_zero()
				&& record.getProfile().getMangaStats().getTotalEntries() < 1) { // if
																				// manga
																				// (total
																				// entry)
																				// is
		...
		if (prefs.anime_manga_zero()
				&& record.getProfile().getAnimeStats().getTotalEntries() < 1) { // if
																				// anime
																				// (total
																				// entry)
																				// is
	...
	public void setcolor() {
		...
		String rank = record.getProfile().getDetails().getAccessRank() != null ? record
				.getProfile().getDetails().getAccessRank()
				: "";
	...
	public void setcolor(boolean type) {
		...
		textview.setTextColor(Color.HSVToColor(new float[] { Hue, 1,
				(float) 0.7 }));
	}
	...
	public void Settext() {
		...
		if (record.getProfile().getDetails().getWebsite() != null
				&& record.getProfile().getDetails().getWebsite()
						.contains("http://")
				&& record.getProfile().getDetails().getWebsite().contains(".")) { // filter
																					// fake
																					// websites
		...
		tv3.setText(String.valueOf(record.getProfile().getDetails()
				.getComments()));
		TextView tv4 = (TextView) findViewById(R.id.forumpostssmall);
		tv4.setText(String.valueOf(record.getProfile().getDetails()
				.getForumPosts()));
		...
		tv9.setText(String.valueOf(record.getProfile().getDetails()
				.getAnimeListViews()));
		TextView tv10 = (TextView) findViewById(R.id.mangalistviewssmall);
		tv10.setText(String.valueOf(record.getProfile().getDetails()
				.getMangaListViews()));

		TextView tv11 = (TextView) findViewById(R.id.atimedayssmall);
		tv11.setText(record.getProfile().getAnimeStats().getTimeDays()
				.toString());
		TextView tv12 = (TextView) findViewById(R.id.awatchingsmall);
		tv12.setText(String.valueOf(record.getProfile().getAnimeStats()
				.getWatching()));
		TextView tv13 = (TextView) findViewById(R.id.acompletedpostssmall);
		tv13.setText(String.valueOf(record.getProfile().getAnimeStats()
				.getCompleted()));
		TextView tv14 = (TextView) findViewById(R.id.aonholdsmall);
		tv14.setText(String.valueOf(record.getProfile().getAnimeStats()
				.getOnHold()));
		TextView tv15 = (TextView) findViewById(R.id.adroppedsmall);
		tv15.setText(String.valueOf(record.getProfile().getAnimeStats()
				.getDropped()));
		TextView tv16 = (TextView) findViewById(R.id.aplantowatchsmall);
		tv16.setText(String.valueOf(record.getProfile().getAnimeStats()
				.getPlanToWatch()));
		TextView tv17 = (TextView) findViewById(R.id.atotalentriessmall);
		tv17.setText(String.valueOf(record.getProfile().getAnimeStats()
				.getTotalEntries()));

		TextView tv18 = (TextView) findViewById(R.id.mtimedayssmall);
		tv18.setText(record.getProfile().getMangaStats().getTimeDays()
				.toString());
		TextView tv19 = (TextView) findViewById(R.id.mwatchingsmall);
		tv19.setText(String.valueOf(record.getProfile().getMangaStats()
				.getReading()));
		TextView tv20 = (TextView) findViewById(R.id.mcompletedpostssmall);
		tv20.setText(String.valueOf(record.getProfile().getMangaStats()
				.getCompleted()));
		TextView tv21 = (TextView) findViewById(R.id.monholdsmall);
		tv21.setText(String.valueOf(record.getProfile().getMangaStats()
				.getOnHold()));
		TextView tv22 = (TextView) findViewById(R.id.mdroppedsmall);
		tv22.setText(String.valueOf(record.getProfile().getMangaStats()
				.getDropped()));
		TextView tv23 = (TextView) findViewById(R.id.mplantowatchsmall);
		tv23.setText(String.valueOf(record.getProfile().getMangaStats()
				.getPlanToRead()));
		TextView tv24 = (TextView) findViewById(R.id.mtotalentriessmall);
		tv24.setText(String.valueOf(record.getProfile().getMangaStats()
				.getTotalEntries()));
	...
	public void refresh(Boolean crouton) {
		if (crouton == true) {
			Crouton.makeText(this, R.string.crouton_info_UserRecord_updated,
					Style.CONFIRM).show();
		}
		if (record == null) {
			if (!MALApi.isNetworkAvailable(context)) {
				Crouton.makeText(this, R.string.crouton_error_noUserRecord,
						Style.ALERT).show();
			} else {
				Crouton.makeText(this, R.string.crouton_error_UserRecord,
						Style.ALERT).show();
			}
		} else {
	...
	void choosedialog(final boolean share) { // as the name says
		...
		builder.setPositiveButton(R.string.dialog_label_anime,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (share == true) {
							...
							Uri mallisturlanime = Uri
									.parse("http://myanimelist.net/animelist/"
											+ record.getName());
		...
		builder.setNeutralButton(R.string.dialog_label_cancel,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
		...
		builder.setNegativeButton(R.string.dialog_label_manga,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						if (share == true) {
							...
							Uri mallisturlmanga = Uri
									.parse("http://myanimelist.net/mangalist/"
											+ record.getName());
							startActivity(new Intent(Intent.ACTION_VIEW,
									mallisturlmanga));
						}
