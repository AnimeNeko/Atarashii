...
public class DatabaseManager {
	...
	public final String[] ANIMECOLUMNS = { "recordID", "recordName",
			"recordType", "recordStatus", "myStatus", "episodesWatched",
			"episodesTotal", "memberScore", "myScore", "synopsis", "imageUrl",
			"dirty", "lastUpdate" };

	private final String[] MANGACOLUMNS = { "recordID", "recordName",
			"recordType", "recordStatus", "myStatus", "volumesRead",
			"chaptersRead", "volumesTotal", "chaptersTotal", "memberScore",
			"myScore", "synopsis", "imageUrl", "dirty", "lastUpdate" };

	private final String[] FRIENDSCOLUMNS = { "username", "avatar_url",
			"last_online", "friend_since" };

	private final String[] PROFILECOLUMNS = { "username", "avatar_url",
			"birthday", "location", "website", "comments", "forum_posts",
			"last_online", "gender", "join_date", "access_rank",
			"anime_list_views", "manga_list_views", "anime_time_days",
			"anime_watching",
			"anime_completed",
			"anime_on_hold",
			"anime_dropped", // anime
			"anime_plan_to_watch", "anime_total_entries", "manga_time_days",
	...
	public Anime getAnime(int id) {
		Anime result = null;
		Cursor cursor = getDBRead().query(MALSqlHelper.TABLE_ANIME,
				ANIMECOLUMNS, "recordID = ?",
	...
	public boolean deleteAnime(int id) {
		return getDBWrite().delete(MALSqlHelper.TABLE_ANIME, "recordID = ?",
	...
	private ArrayList<Anime> getAnimeList(String selection,
			String[] selectionArgs) {
		ArrayList<Anime> result = null;
		Cursor cursor;
		try {
			cursor = getDBRead().query(MALSqlHelper.TABLE_ANIME, ANIMECOLUMNS,
					selection, selectionArgs, null, null, "recordName ASC");
			if (cursor.moveToFirst()) {
			...
			Log.e("MALX",
					"DatabaseManager.getAnimeList exception: " + e.getMessage());
		}
	...
	public int clearOldAnimeRecords(Date time) {
		return getDBWrite().delete(MALSqlHelper.TABLE_ANIME, "lastUpdate < ?",
	...
	public Manga getManga(int id) {
		Manga result = null;
		Cursor cursor = getDBRead().query(MALSqlHelper.TABLE_MANGA,
				MANGACOLUMNS, "recordID = ?",
				new String[] { Integer.toString(id) }, null, null, null);
		if (cursor.moveToFirst())
	...
	public boolean deleteManga(int id) {
		return getDBWrite().delete(MALSqlHelper.TABLE_MANGA, "recordID = ?",
				new String[] { String.valueOf(id) }) == 1;
	}
	...
	private ArrayList<Manga> getMangaList(String selection,
			String[] selectionArgs) {
		ArrayList<Manga> result = null;
		Cursor cursor;
		try {
			cursor = getDBRead().query(MALSqlHelper.TABLE_MANGA, MANGACOLUMNS,
					selection, selectionArgs, null, null, "recordName ASC");
			if (cursor.moveToFirst()) {
			...
			Log.e("MALX",
					"DatabaseManager.getMangaList exception: " + e.getMessage());
		}
	...
	public int clearOldMangaRecords(Date time) {
		return getDBWrite().delete(MALSqlHelper.TABLE_MANGA, "lastUpdate < ?",
				new String[] { time.toString() });
	}
	...
	public void saveFriendList(ArrayList<User> list) {
		if (list != null && list.size() > 0) {
			try {
				...
				Log.e("MALX",
						"error saving friendlist to db: " + e.getMessage());
			} finally {
				getDBWrite().endTransaction();
			}
		}
	...
	public void saveFriend(User friend) {
		...
		if (friend.getProfile().getDetails().getLastOnline() != null)
			cv.put("last_online", friend.getProfile().getDetails()
					.getLastOnline());
		else
	...
	public ArrayList<User> getFriendList() {
		...
		Cursor cursor;
		try {
			cursor = getDBRead().query(MALSqlHelper.TABLE_FRIENDS,
					FRIENDSCOLUMNS, null, null, null, null, "username ASC");
			if (cursor.moveToFirst()) {
			...
			Log.e("MALX",
					"DatabaseManager.getFriendList exception: "
							+ e.getMessage());
		}
	...
	public void saveUser(User user) {
		...
		if (user.getProfile().getDetails().getLastOnline() != null)
			cv.put("last_online", user.getProfile().getDetails()
					.getLastOnline());
		else
			cv.putNull("last_online");
		...
		cv.put("anime_list_views", user.getProfile().getDetails()
				.getAnimeListViews());
		cv.put("manga_list_views", user.getProfile().getDetails()
				.getMangaListViews());

		cv.put("anime_time_days", user.getProfile().getAnimeStats()
				.getTimeDays());
		cv.put("anime_watching", user.getProfile().getAnimeStats()
				.getWatching());
		cv.put("anime_completed", user.getProfile().getAnimeStats()
				.getCompleted());
		...
		cv.put("anime_plan_to_watch", user.getProfile().getAnimeStats()
				.getPlanToWatch());
		cv.put("anime_total_entries", user.getProfile().getAnimeStats()
				.getTotalEntries());

		cv.put("manga_time_days", user.getProfile().getMangaStats()
				.getTimeDays());
		cv.put("manga_reading", user.getProfile().getMangaStats().getReading());
		cv.put("manga_completed", user.getProfile().getMangaStats()
				.getCompleted());
		...
		cv.put("manga_plan_to_read", user.getProfile().getMangaStats()
				.getPlanToRead());
		cv.put("manga_total_entries", user.getProfile().getMangaStats()
				.getTotalEntries());
	...
	public User getProfile(String name) {
		...
		Cursor cursor;
		try {
			cursor = getDBRead().query(MALSqlHelper.TABLE_PROFILE,
					PROFILECOLUMNS, "username = ?", new String[] { name },
					null, null, null);
			if (cursor.moveToFirst())
			...
			Log.e("MALX",
					"DatabaseManager.getProfile exception: " + e.getMessage());
		}
		return result;
	}
}
