...
public class PrefManager {
	...
	public String getCustomShareText() {
		return prefs.getString("customShareText",
				"Check out $title; on MyAnimeList!\n$link;");
	}
	...
	public boolean getonly_wifiEnabled() { // Home, if the setting sync only at
											// wifi is turned on
	...
	public Integer getsync_time_last() { // Home, get the last auto-sync
											// interval
		return prefs.getInt("synchronisation_time_last", 1);
	}

	public boolean anime_manga_zero() { // profile activity, if the card is
										// empty setting
		return prefs.getBoolean("a_mhide", false);
	}

	public boolean Textcolordisable() { // profile activity, if the textcolors
										// are turned off
		return prefs.getBoolean("text_colours", false);
	}

	public boolean animehide() { // profile activity, if the setting force hide
									// is turned on
		return prefs.getBoolean("A_hide", false); // anime card
	}

	public boolean mangahide() {// profile activity, if the setting force hide
								// is turned on
	...
	public void setsync_time_last(int time) { // Home, set the last auto-sync
												// interval
	...
	public long getSyncFrequency() {
		...
		syncFrequency = Long.parseLong(prefs.getString("syncFrequency",
				"604800000"));
