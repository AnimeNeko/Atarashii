...
public class MALApi {
	...
	private static final String USER_AGENT = "Atarashii! (Linux; Android "
	...
	private void setupRESTService(String username, String password) {
		...
		RestAdapter restAdapter = new RestAdapter.Builder()
				.setClient(new ApacheClient(client)).setServer(API_HOST)
				.build();
	...
	public boolean isAuth() {
		try {
			...
			if (e.getResponse() != null)
				Log.e("MALX", "caught retrofit error: "
	...
	public boolean addOrUpdateAnime(Anime anime) {
		...
		if (anime.getCreateFlag())
			result = service.addAnime(anime.getId(), anime.getWatchedStatus(),
					anime.getWatchedEpisodes(), anime.getScore()).getStatus() == 200;
		else
			result = service.updateAnime(anime.getId(),
					anime.getWatchedStatus(), anime.getWatchedEpisodes(),
					anime.getScore()).getStatus() == 200;
		return result;
	}
	...
	public boolean addOrUpdateManga(Manga manga) {
		...
		if (manga.getCreateFlag())
			result = service.addManga(manga.getId(), manga.getReadStatus(),
					manga.getChaptersRead(), manga.getVolumesRead(),
					manga.getScore()).getStatus() == 200;
		else
			result = service.updateManga(manga.getId(), manga.getReadStatus(),
					manga.getChaptersRead(), manga.getVolumesRead(),
					manga.getScore()).getStatus() == 200;
		return result;
	}
