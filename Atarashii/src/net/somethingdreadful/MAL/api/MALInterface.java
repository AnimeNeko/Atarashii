...
public interface MALInterface {
	...
	Response addAnime(@Field("anime_id") int id,
			@Field("status") String status, @Field("episodes") int episodes,
			@Field("score") int score);

	@FormUrlEncoded
	@PUT("/animelist/anime/{anime_id}")
	Response updateAnime(@Path("anime_id") int id,
			@Field("status") String status, @Field("episodes") int episodes,
			@Field("score") int score);
	...
	Response addManga(@Field("manga_id") int id,
			@Field("status") String status, @Field("chapters") int chapters,
			@Field("volumes") int volumes, @Field("score") int score);

	@FormUrlEncoded
	@PUT("/mangalist/manga/{manga_id}")
	Response updateManga(@Path("manga_id") int id,
			@Field("status") String status, @Field("chapters") int chapters,
			@Field("volumes") int volumes, @Field("score") int score);
