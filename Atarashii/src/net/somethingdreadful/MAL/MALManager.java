...
public class MALManager {
	 ...
	 * addItemToMal sets score to 0... so do that and pass to
	 * writeAnimeDetailsToMAL
	 */
	...
	public void clearDeletedItems(String type, Date currentTime) {
		Log.v("MALX", "Removing deleted items of type " + type + " older than "
	...
	public boolean cleanDirtyAnimeRecords() {
		...
		if (dirtyAnimes != null) {
			Log.v("MALX", "Got " + dirtyAnimes.size()
					+ " dirty anime records. Cleaning..");
			...
			Log.v("MALX", "Cleaned dirty anime records, status: "
					+ totalSuccess);
		}
		return totalSuccess;
	}
	...
	public boolean cleanDirtyMangaRecords() {
		...
		if (dirtyMangas != null) {
			Log.v("MALX", "Got " + dirtyMangas.size()
					+ " dirty manga records. Cleaning..");
			...
			Log.v("MALX", "Cleaned dirty manga records, status: "
					+ totalSuccess);
		}
		return totalSuccess;
	}
