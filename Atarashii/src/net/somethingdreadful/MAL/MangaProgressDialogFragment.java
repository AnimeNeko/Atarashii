...
public class MangaProgressDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		view = View.inflate(new ContextThemeWrapper(getActivity(),
				R.style.AlertDialog), R.layout.dialog_manga_progress, null);
		...
		builder.setPositiveButton("Update",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int whichButton) {
						((DetailView) getActivity()).onMangaDialogDismissed(
								chapterPicker.getValue(),
								volumePicker.getValue());
						dismiss();
					}
				})
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (state == null) {
			...
			chaptersTotal = ((DetailView) getActivity()).mangaRecord
					.getChapters();
			chaptersRead = ((DetailView) getActivity()).mangaRecord
					.getChaptersRead();
			chapterPickerValue = chaptersRead;

			volumesTotal = ((DetailView) getActivity()).mangaRecord
					.getVolumes();
			volumesRead = ((DetailView) getActivity()).mangaRecord
					.getVolumesRead();
