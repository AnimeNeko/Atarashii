...
public class DetailView extends SherlockFragmentActivity implements
		DetailsBasicFragment.IDetailsBasicAnimeFragment,
		EpisodesPickerDialogFragment.DialogDismissedListener,
		MangaProgressDialogFragment.MangaDialogDismissedListener,
		StatusPickerDialogFragment.StatusDialogDismissedListener,
		RatingPickerDialogFragment.RatingDialogDismissedListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		recordID = getIntent().getIntExtra(
				"net.somethingdreadful.MAL.recordID", 1);

		// Get the recordType, also passed from calling activity
		// Record type will determine how the detail view lays out itself
		recordType = getIntent().getStringExtra(
				"net.somethingdreadful.MAL.recordType");
		...
		bfrag = (DetailsBasicFragment) fm
				.findFragmentById(R.id.DetailsFragment);

		SynopsisFragment = (GenericCardFragment) fm
				.findFragmentById(R.id.SynopsisFragment);
		SynopsisFragment.setArgsSensibly(
				getString(R.string.card_name_synopsis),
				R.layout.card_layout_content_synopsis,
				GenericCardFragment.CONTENT_TYPE_SYNOPSIS, false);
		SynopsisFragment.inflateContentStub();

		ProgressFragment = (GenericCardFragment) fm
		...
		ProgressFragment.setArgsSensibly(
				getString(R.string.card_name_progress), card_layout_progress,
				GenericCardFragment.CONTENT_TYPE_PROGRESS, true);
		...
		ProgressFragment.getView().setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO: Added some kind of clicking feedback, like highlight it
				// or something
				showProgressDialog();
			}
		...
		StatusFragment = (GenericCardFragment) fm
				.findFragmentById(R.id.StatusFragment);
		StatusFragment.setArgsSensibly(getString(R.string.card_name_stats),
				R.layout.card_layout_status,
				GenericCardFragment.CONTENT_TYPE_INFO, false);
		StatusFragment.inflateContentStub();

		ScoreFragment = (GenericCardFragment) fm
				.findFragmentById(R.id.ScoreFragment);
		ScoreFragment.setArgsSensibly(getString(R.string.card_name_rating),
				R.layout.card_layout_score,
				GenericCardFragment.CONTENT_TYPE_SCORE, true);
		...
		WatchStatusFragment = (GenericCardFragment) fm
				.findFragmentById(R.id.WatchStatusFragment);
		WatchStatusFragment.setArgsSensibly(
				getString(R.string.card_name_status),
				R.layout.card_layout_watchstatus,
				GenericCardFragment.CONTENT_TYPE_WATCHSTATUS, true);
	...
	private void setupBeam() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			...
			if (mNfcAdapter == null) {
				...
				String message_str = recordType + ":"
						+ String.valueOf(recordID);
				NdefMessage message = new NdefMessage(
						new NdefRecord[] {
								new NdefRecord(NdefRecord.TNF_MIME_MEDIA,
										"application/net.somethingdreadful.MAL"
												.getBytes(Charset
														.forName("US-ASCII")),
										new byte[0], message_str
												.getBytes(Charset
														.forName("US-ASCII"))),
								NdefRecord
										.createApplicationRecord("net.somethingdreadful.MAL") });
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		if (isAdded) {
			...
			getSupportMenuInflater().inflate(
					R.menu.activity_detail_view_unrecorded, menu);
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_ViewMALPage:
			Uri malurl = Uri.parse("http://myanimelist.net/" + recordType + "/"
					+ recordID + "/");
	...
	public void showRemoveDialog() {
		...
		if (Build.VERSION.SDK_INT >= 11) {
			rcdf.setStyle(SherlockDialogFragment.STYLE_NORMAL,
	...
	// Called after the basic fragment is finished it's setup, populate data
	// into it
	@Override
	public void basicFragmentReady() {

		CoverImageView = (ImageView) bfrag.getView().findViewById(
				R.id.detailCoverImage);
	...
	public void showProgressDialog() // Just a function to keep logic out of the
										// switch statement
	{
		if ("anime".equals(recordType)) {
	...
	public void showStatusDialog() {
		...
		if (Build.VERSION.SDK_INT >= 11) {
			spdf.setStyle(SherlockDialogFragment.STYLE_NORMAL,
					android.R.style.Theme_Holo_Dialog);
		} else {
	...
	public void showRatingDialog() {
		...
		if (Build.VERSION.SDK_INT >= 11) {
			rpdf.setStyle(SherlockDialogFragment.STYLE_NORMAL,
					android.R.style.Theme_Holo_Dialog);
		} else {
	...
	public void showEpisodesWatchedDialog() {
		...
		// Note we use setStyle to change the theme, the default light styled
		// dialog didn't look good so we use the dark dialog
		epd = new EpisodesPickerDialogFragment();

		if (Build.VERSION.SDK_INT >= 11) {
			epd.setStyle(SherlockDialogFragment.STYLE_NORMAL,
					android.R.style.Theme_Holo_Dialog);
		} else {
	...
	public void showMangaProgressDialog() {
		// Standard code for setting up a dialog fragment
		// Toast.makeText(context, "TODO: Make a MangaProgressFragment",
		// Toast.LENGTH_SHORT).show();
		mpdf = new MangaProgressDialogFragment();
		if (Build.VERSION.SDK_INT >= 11) {
			mpdf.setStyle(SherlockDialogFragment.STYLE_NORMAL,
					android.R.style.Theme_Holo_Dialog);
		} else {
	...
	public class getDetailsTask extends AsyncTask<Void, Boolean, GenericRecord> {
		...
		protected GenericRecord doInBackground(Void... arg0) {
			...
			if ("anime".equals(internalType)) {
				...
				// Basically I just use publishProgress as an easy way to
				// display info we already have loaded sooner
				// This way, I can let the database work happen on the
				// background thread and then immediately display it while
				// the synopsis loads if it hasn't previously been downloaded.
				publishProgress(true);

				if (networkAvailable && animeRecord != null) {
					if ((animeRecord.getSynopsis() == null)
							|| (animeRecord.getMembersScore() <= 0)) {
						animeRecord = mMalManager.updateWithDetails(mRecordID,
								animeRecord);
					}
				} else {
				...
				if (networkAvailable && mangaRecord != null) {
					if ((mangaRecord.getSynopsis() == null)
							|| (mangaRecord.getMembersScore() <= 0)) {
						mangaRecord = mMalManager.updateWithDetails(mRecordID,
								mangaRecord);
					}
				}
		...
		protected void onProgressUpdate(Boolean... progress) {
			if (animeRecord != null
					&& MALManager.TYPE_ANIME.equals(internalType)) {
				...
				coverImage.load(animeRecord.getImageUrl())
						...
						.placeholder(R.drawable.cover_loading).fit()
						.into(CoverImageView);

				ProgressText = Integer.toString(animeRecord
						.getWatchedEpisodes());
				TotalProgressText = Integer.toString(animeRecord.getEpisodes());
				MyStatusText = WordUtils.capitalize(animeRecord
						.getWatchedStatus());

				ProgressCurrentView = (TextView) ProgressFragment.getView()
						.findViewById(R.id.progressCountCurrent);
				ProgressTotalView = (TextView) ProgressFragment.getView()
						.findViewById(R.id.progressCountTotal);
				...
				RecordStatusText = WordUtils
						.capitalize(animeRecord.getStatus());
				...
				RecordTypeView = (TextView) StatusFragment.getView()
						.findViewById(R.id.mediaType);
				RecordStatusView = (TextView) StatusFragment.getView()
						.findViewById(R.id.mediaStatus);
				...
				if (animeRecord.getWatchedStatus() == null) {
					Log.v("MALX",
							"No status found; Record must have been searched for, therefore not added to list");
					setAddToListUI(true);
				}
				...
				VolumeProgressText = Integer.toString(mangaRecord
						.getVolumesRead());
				...
				MyStatusText = WordUtils
						.capitalize(mangaRecord.getReadStatus());

				ProgressCurrentVolumeView = (TextView) ProgressFragment
						.getView().findViewById(
								R.id.progressVolumesCountCurrent);
				ProgressTotalVolumeView = (TextView) ProgressFragment.getView()
						.findViewById(R.id.progressVolumesCountTotal);
				ProgressCurrentView = (TextView) ProgressFragment.getView()
						.findViewById(R.id.progressCountCurrent);
				ProgressTotalView = (TextView) ProgressFragment.getView()
						.findViewById(R.id.progressCountTotal);
				MyStatusView = (TextView) WatchStatusFragment.getView()
						.findViewById(R.id.cardStatusLabel);
				...
				RecordStatusText = WordUtils
						.capitalize(mangaRecord.getStatus());
		...
		protected void onPostExecute(GenericRecord gr) {
			if (gr != null) {
				...
				if ("anime".equals(internalType)) {
					MALScoreBar = (RatingBar) ScoreFragment.getView()
							.findViewById(R.id.MALScoreBar);
					MyScoreBar = (RatingBar) ScoreFragment.getView()
							.findViewById(R.id.MyScoreBar);
				...
				if (SynopsisFragment.getView() != null) {
					SynopsisView = (TextView) SynopsisFragment.getView()
							.findViewById(R.id.SynopsisContent);

					if (SynopsisView != null) {
						SynopsisView.setText(SynopsisText,
				...
				// if gr is null then the anime/manga is not stored in the
				// database and could not be loaded from the API (e. g. no
				// network connection)
				Toast.makeText(context, R.string.crouton_error_DetailsError,
						Toast.LENGTH_SHORT).show();
				finish();
			}
		}
	...
	public class writeDetailsTask extends
			AsyncTask<GenericRecord, Void, Boolean> {
		...
		protected Boolean doInBackground(GenericRecord... gr) {
			...
			if (gr[0].getDeleteFlag()) {
				if ("anime".equals(internalType)) {
					...
					result = internalManager
							.writeAnimeDetailsToMAL((Anime) gr[0]);
				} else {
					internalManager.deleteMangaFromDatabase((Manga) gr[0]);
					result = internalManager
							.writeMangaDetailsToMAL((Manga) gr[0]);
				}
			} else {
				...
				if (result) {
					...
					if ("anime".equals(internalType)) {
						internalManager.saveAnimeToDatabase((Anime) gr[0],
								false);
					} else {
						internalManager.saveMangaToDatabase((Manga) gr[0],
								false);
					}
			...
			if (newValue == animeRecord.getWatchedEpisodes()) {
				...
				if (animeRecord.getEpisodes() != 0) {
					if (newValue == animeRecord.getEpisodes()) {
						...
						MyStatusView.setText(WordUtils
								.capitalize(GenericRecord.STATUS_COMPLETED));
					}
					if (newValue == 0) {
						animeRecord.setWatchedStatus(Anime.STATUS_PLANTOWATCH);
						MyStatusView.setText(WordUtils
								.capitalize(Anime.STATUS_PLANTOWATCH));
					}
	...
	public void setAddToListUI(boolean enabled) {
		if (enabled) { // Configure DetailView to show the add to list UI, hide
						// extraneous elements
			isAdded = false;
	...
	public void onMangaDialogDismissed(int newChapterValue, int newVolumeValue) {
		...
		if ("manga".equals(recordType)) {
			...
			if (newVolumeValue == mangaRecord.getVolumesRead()) {
				...
				ProgressCurrentVolumeView.setText(Integer
						.toString(newVolumeValue));
			}
	...
	public void contentInflated(int contentType) {
		...
		case GenericCardFragment.CONTENT_TYPE_SYNOPSIS:
			SynopsisView = (TextView) SynopsisFragment.getView().findViewById(
					R.id.SynopsisContent);
		...
		case GenericCardFragment.CONTENT_TYPE_INFO:
			RecordTypeView = (TextView) StatusFragment.getView().findViewById(
					R.id.mediaType);
			RecordStatusView = (TextView) StatusFragment.getView()
					.findViewById(R.id.mediaStatus);
		...
		case GenericCardFragment.CONTENT_TYPE_SCORE:
			MALScoreBar = (RatingBar) ScoreFragment.getView().findViewById(
					R.id.MALScoreBar);
			MyScoreBar = (RatingBar) ScoreFragment.getView().findViewById(
					R.id.MyScoreBar);
			if (MemberScore > 0) {
				MALScoreBar.setRating(MemberScore / 2);
				MyScoreBar.setRating(MyScore / 2);
	...
	public String makeShareText() {
		...
		shareText = shareText.replace("$link;", "http://myanimelist.net/"
				+ recordType + "/" + Integer.toString(recordID));

		shareText = shareText
				+ getResources().getString(
						R.string.customShareText_fromAtarashii);
